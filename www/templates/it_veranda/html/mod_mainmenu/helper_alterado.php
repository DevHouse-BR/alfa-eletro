 ...
 * @since		1.5
 */
class IcemodMainMenuHelper
{
	function IcebuildXML(&$params)
	{
		
		$menu = new IceJMenuTree($params);
		$items = &JSite::getMenu();

		// Get Menu Items
		//$rows = $items->getItems('menutype', $params->get('menutype'));
		
		//$rows = menuProductCategories();
		
		$mymenu_content = "";
		
		traverse_tree_down($mymenu_content);
		
		print_r($mymenu_content);
	...
	function render(&$params, $callback)
	{
		switch ( $params->get( 'menu_style', 'list' ) )
		{
			...
			default :
				...
				$xml = IcemodMainMenuHelper::getXML($params->get('menutype'), $params, $callback);
				if ($xml) {
 ...
 * @since		1.5
 */
class IceJMenuTree extends JTree
{
	/**
 ...
 * @since		1.5
 */
class IceJMenuNode extends JNode
{
	/**
	 * Node Title
	 */
...
function traverse_tree_down(&$mymenu_content, $category_id='0', $level='0') {
		...
		while($db->next_record()) {
			...
			traverse_tree_down($mymenu_content, $db->f("category_child_id"), $level);
