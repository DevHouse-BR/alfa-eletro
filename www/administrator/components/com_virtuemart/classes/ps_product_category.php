...
class ps_product_category extends vmAbstractObject {
	...
	*/
	...
	function get_subcategories( $category_id=0,
				$links_css_class="mainlevel",
				$list_css_class="mm123",
		...
		for($n = 0 ; $n < $nrows ; $n++) {
			...
			if( $allowed ) {
				...
				if( $depth_list[$n] > 0 ) {
					...
					$catname = shopMakeHtmlSafe( $category_tmp[$row_list[$n]]["category_name"] );
				$html .= '
					<a title="'.$catname.'" style="margin-right:6px;'.$style.'" class="'. $css_class .'" href="'. $sess->url(URL."index.php?page=shop.browse&amp;category_id=".$category_tmp[$row_list[$n]]["category_child_id"]) .'" '.$append.'>'
					. $catname
					. ps_product_category::products_in_category( $category_tmp[$row_list[$n]]["category_child_id"] )
					.'</a>';
				}
