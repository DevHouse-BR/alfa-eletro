...
class PhocaDownloadCpControllerPhocaDownload extends PhocaDownloadCpController
{
	...
	function save() {
		...
		switch ( JRequest::getCmd('task') ) {
			case 'apply':
				...
				if ($id && $id > 0) {
					$msg = JText::_( 'Changes to Phoca Download Saved' );
					//$id		= $model->store($post);
				} else {
			...
			default:
				if ($model->store($post)) {
					$msg = JText::_( 'Phoca Download Saved' );
				} else {
					$msg = JText::_( 'Error Saving Phoca Download ' );
				}
	...
	function remove() {
		...
		if(!$model->delete($cid)) {
			...
			$msg = JText::_( 'Error Deleting Phoca Download' );
		}
		else {
			$msg = JText::_( 'Phoca Download Deleted' );
		}
