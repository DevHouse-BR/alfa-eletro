 ...
 * @version $Id: importfile.php 907 2009-05-22 15:26:28Z Roland $
 */
...
class CsvimprovedModelImportfile extends JModel {
	...
	public function getCheckLimits() {
		...
		if (!function_exists('memory_get_usage')) return true;
			...
			else JRequest::setVar('maxmem', $totalmem);
	...
	*	Check user options for import
	...
	function getImportDetails() {
		...
		/* Show the file path */
		$csvilog->AddMessage('debug', JText::_('DEBUG_FILE_PATH').' '.$template->file_location);
	}
	 ...
	 * @see csv_category()
  	...
  	function CheckCategoryPath() {
		...
		if (($this->product_details->category_path && !$this->product_details->child_product) || $this->product_details->category_id) {
			$csvilog->AddMessage('debug', 'Checking category path');
			...
			if (!$this->product_details->product_parent_id || $this->product_details->category_id) {
				...
				$csvilog->AddMessage('debug', 'Delete old category references: <a onclick="switchMenu(\''.$this->product_details->product_sku.'_delete_old_cat_xref\');" title="Show/hide query">Show/hide query</a><div id="'.$this->product_details->product_sku.'_delete_old_cat_xref" class="debug">'.htmlentities($q).'</div>');
				$dbcat->query($q);
				...
				foreach( $this->product_details->category_id as $value ) {
					...
					$csvilog->AddMessage('debug', 'Add new category references: <a onclick="switchMenu(\''.$this->product_details->product_sku.'_add_new_cat_xref'.$value.'\');" title="Show/hide query">Show/hide query</a><div id="'.$this->product_details->product_sku.'_add_new_cat_xref'.$value.'" class="debug">'.htmlentities($q).'</div>');
					if (!$dbcat->query($q)) $csvilog->AddMessage('debug', '<span class="sqlerror">SQL Error</span>: <a onclick="switchMenu(\''.$this->product_details->product_sku.'_product_category_xref_error\');" title="Show/hide error">Show/hide error</a><div id="'.$this->product_details->product_sku.'_product_category_xref_error" class="debug">'.mysql_error().'</div>');
				}
			}
		}
		else {
		...
		foreach( $categories as $line ) {
			...
			for($i = 0; $i < $category_count; $i++) {
				...
				/* Category does not exist - create it */
					...
					$csvilog->AddMessage('debug', 'Add new category: <a onclick="switchMenu(\''.$db->getEscaped($category_list[$i]).'_add_category\');" title="Show/hide query">Show/hide query</a><div id="'.$db->getEscaped($category_list[$i]).'_add_category" class="debug">'.htmlentities($q).'</div>');
					$db->setQuery($q);
					if (!$db->query()) $csvilog->AddMessage('debug', '<span class="sqlerror">SQL Error</span>: <a onclick="switchMenu(\''.$db->getEscaped($category_list[$i]).'_category_error\');" title="Show/hide error">Show/hide error</a><div id="'.$category_list[$i].'_category_error" class="debug">'.mysql_error().'</div>');
					...
					$q .= $category_id . "')";
					$csvilog->AddMessage('debug', 'Add new category xref: <a onclick="switchMenu(\''.$db->getEscaped($category_list[$i]).'_add_category_xref\');" title="Show/hide query">Show/hide query</a><div id="'.$db->getEscaped($category_list[$i]).'_add_category_xref" class="debug">'.htmlentities($q).'</div>');
					$db->setQuery($q);
					if (!$db->query()) $csvilog->AddMessage('debug', '<span class="sqlerror">SQL Error</span>: <a onclick="switchMenu(\''.$db->getEscaped($category_list[$i]).'_category_xref_error\');" title="Show/hide error">Show/hide error</a><div id="'.$category_list[$i].'_category_xref_error" class="debug">'.mysql_error().'</div>');
				}
