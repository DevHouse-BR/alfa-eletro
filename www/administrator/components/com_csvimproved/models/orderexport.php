 ...
 * @version $Id: orderexport.php 902 2009-05-10 01:50:10Z Suami $
 */
		...
		foreach ($required_fields as $column_header => $fieldname) {
			switch ($fieldname) {
				...
				case 'mdate':
					...
					break;
				case 'full_name':
					if (!in_array('first_name', $required_fields)) $userfields[] = '#__vm_order_user_info.first_name';
					if (!in_array('middle_name', $required_fields)) $userfields[] = '#__vm_order_user_info.middle_name';
					if (!in_array('last_name', $required_fields)) $userfields[] = '#__vm_order_user_info.last_name';
					break;
				...
				case 'total_order_items':
					...
					break;
				default:
					if ($address == 'BTST' && array_key_exists($fieldname, $user_info_fields)) {
						$userfields[] = 'COALESCE(user_info2.'.$fieldname.', user_info1.'.$fieldname.') AS '.$fieldname;
					}
					else $userfields[] = $fieldname;
					break;
			}
		}
		...
		if ($address) {
			switch (strtoupper($address)) {
				case 'BTST':
					...
					break;
				default:
					$selectors[] = '#__vm_order_user_info.address_type = '.$db->Quote(strtoupper($address));
					break;
			}
		}
			...
			foreach ($required_fields as $column_header => $fieldname) {
				switch ($fieldname) {
					...
					case 'payment_method_id':
						...
						break;
					default:
						if ($address == 'BTST' && array_key_exists($fieldname, $user_info_fields)) $q .= 'user_info1.'.$fieldname.", ";
						else $q .= $fieldname.", ";
						break;
				}
			}
				...
				foreach ($required_fields as $column_header => $fieldname) {
					switch ($fieldname) {
						...
						case 'order_id':
							...
							if (in_array('total_order_items', $required_fields)) {
