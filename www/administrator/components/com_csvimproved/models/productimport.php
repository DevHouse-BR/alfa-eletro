 ...
 * @version $Id: productimport.php 907 2009-05-22 15:26:28Z Roland $
 */
...
class CsvimprovedModelProductimport extends JModel {
			...
			/* User wants to add or update the product */
				...
				if ($template->thumb_create) {
					...
					if (isset($this->product_full_image) && isset($this->product_thumb_image)) {
						$file_details = $this->_importmodel->ProcessImage($this->product_full_image, $this->product_thumb_image);
					}
					else {
				...
				if (isset($csv_fields['product_tax'])) $this->ProcessTax();
				...
				else if (!isset($csv_fields['manufacturer_name']) && !isset($csv_fields['manufacturer_id'])) {
				...
				if ($this->ProductQuery()) {
					...
					if (isset($csv_fields['product_price']) || isset($csv_fields['price_with_tax'])) $this->PriceQuery();
					...
					*/
					...
					if (isset($csv_fields["category_path"]) || isset($csv_fields["category_id"])) {
						if (($this->category_ids && !$this->child_product) || $this->category_id) {
							$categorymodel = new CsvimprovedModelCategory();
							$categorymodel->getStart();
							if (!is_null($this->category_ids) && isset($csv_fields["category_path"])) $categorymodel->CheckCategoryPath($this->product_id, $this->category_ids, false, $this->product_list);
							else if (!is_null($this->category_id) && isset($csv_fields["category_id"])) {
   ...
   public function getPostProcessing() {
	...
	function ProductQuery() {
	  ...
	  $this->_vm_product->setValue('mdate', time());
	  
	  /* Add a creating date if there is no product_id */
	  if (is_null($this->product_id)) $this->_vm_product->setValue('cdate', time());
	  foreach ($supportedfields as $id => $column) {
         ...
         if (isset($csv_fields[$column])) {
			...
			if ($column == "product_cdate" && !is_null($this->product_id)) {
				$this->_vm_product->setValue('cdate', $this->$column);
			}
			
			/* Add a redirect for the product mdate */
			if ($column == "product_mdate" && !empty($this->$column)) {
				$this->_vm_product->setValue('mdate', $this->$column);
			}
		 }
	  }
	  ...
	  if ($this->_vm_product->store()) {
		  ...
		  $this->product_id = $this->_vm_product->getValue('product_id');
		  return true;
	  }
	  else {
		  $csvilog->AddMessage('debug', JText::_('DEBUG_STORE_PRODUCT'), true);
   ...
   function PostProcessRelatedProducts() {
	  ...
	  if (isset($csv_fields['related_products'])) {
		  $newrelations = array();
   ...
   function ProcessTax() {
		...
		if (isset($csv_fields["product_tax_id"])) $this->_vm_tax_rate->setValue('tax_rate_id', $this->product_tax_id);
		if (isset($csv_fields["product_tax"])) $this->_vm_tax_rate->setValue('tax_rate', $this->product_tax/100);
	 ...
	 *
	 * @link http://www.csvimproved.com/wiki/doku.php/csvimproved:availablefields:attribute_values
	 */
