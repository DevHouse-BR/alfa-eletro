 ...
 * @version $Id: import.php 907 2009-05-22 15:26:28Z Roland $
 */
...
class CsvimprovedModelImport extends JModel {
	 ...
	 */
	public function ProcessImage($file_name_original, $file_name_thumb) {
		...
		if ($template->thumb_width > 0 && $template->thumb_height > 0) {
			...
			$file_details = $this->FileDetails($file_name_original, $file_name_thumb);
			...
			if ($file_details) {
				...
				if (substr($file_name_original, 0, 4) == 'http') {
					...
					if ($template->collect_debug_info) {
						$csvilog->AddMessage('debug', JText::_('Retrieving external file:').' '.$file_name_original.'<br />'.JText::_('Saving remote file to local file:').' '.$file_details['file_path'].DS.$file_details['file_name']);
					}
					JFile::write($file_details['file_path'].DS.$file_details['file_name'], JFile::read($file_name_original));
				...
				if ($template->collect_debug_info) {
					...
					$csvilog->AddMessage('debug', JText::_('DEBUG_CHECK_FILE_EXISTS<br />File:').' '.$file_details['file_path'].DS.$file_details['file_name']);
				}
				...
				if ($file_details['file_is_image']) {
					...
					$find_delim = array("/", "\\");
					$thumb_file_details['file_out'] = str_replace($find_delim, DS, $template->file_location).DS.'resized'.DS.$thumb_file_name;
	 ...
	 */
	public function FileDetails($file_name_original, $file_name_thumb) {
		$template = JRequest::getVar('template');
		...
		/* Get the file details */
		$file_name = basename($clean_file_name_original);
		
		if (substr($file_name_original, 0, 4) == 'http') {
			$file_path = JPATH_CACHE.DS.'csvi';
		}
		else {
			if (strstr($clean_file_name_original, DS)) $dirname = DS.dirname($clean_file_name_original);
			else $dirname = '';
			$file_path = str_replace($find_delim, DS, $template->file_location).$dirname;
		}
		...
		if (JFile::exists($file_path.DS.$file_name)) {
			...
			$file_details['file_name'] = $file_name_original;
