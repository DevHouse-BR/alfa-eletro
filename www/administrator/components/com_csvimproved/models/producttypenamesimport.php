 ...
 * @version $Id: producttypenamesimport.php 907 2009-05-22 15:26:28Z Roland $
 */
			...
			if (is_array($columns)) {
				...
				if ($product_type_exists > 0) {
					...
					foreach ($columns as $key => $colname) {
						if (isset($csv_fields[strtolower($colname)])) {
							$q .= $db->nameQuote($colname)." = ".$db->Quote($product_data[$csv_fields[strtolower($colname)]['order']]).",";
						}
					}
					$q = substr($q, 0, -1)." ";
					...
					foreach ($columns as $key => $colname) {
						if (isset($csv_fields[strtolower($colname)])) {
							...
							$qvalues .= $db->Quote($product_data[$csv_fields[strtolower($colname)]['order']]).',';
						}
