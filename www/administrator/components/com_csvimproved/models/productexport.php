 ...
 * @version $Id: productexport.php 902 2009-05-10 01:50:10Z Suami $
 */
...
class CsvimprovedModelProductExport extends JModel {
	...
	public function getStart() {
		...
		if (array_key_exists("attributes", $required_fields) && !isset($required_fields["product_parent_sku"])) 
			$required_fields["product_parent_sku"] = 0;
		else if (array_key_exists("attribute_values", $required_fields) && !isset($required_fields["product_parent_sku"])) 
			$required_fields["product_parent_sku"] = 0;
				...
				foreach ($required_fields as $column_header => $fieldname) {
					switch ($fieldname) {
						case 'category_path':
							...
							if (strlen(trim($category_path)) == 0) $category_path = $default_values[$column_header];
							$contents .= $this->_exportmodel->AddExportField($category_path, $fieldname, $column_header);
							break;
						case 'attributes':
							...
							else $attributes = $default_values[$column_header];
							$contents .= $this->_exportmodel->AddExportField(trim($attributes), $fieldname, $column_header);
							break;
						case 'attribute_values':
							...
							else $attribute_values = $default_values[$column_header];
							$contents .= $this->_exportmodel->AddExportField(trim($attribute_values), $fieldname, $column_header);
							break;
						case 'product_parent_sku':
							$db->setQuery( "SELECT product_sku FROM #__vm_product WHERE product_id='".$record->product_parent_id."'" );
							$contents .= $this->_exportmodel->AddExportField($db->loadResult(), $fieldname, $column_header);
							break;
						case 'product_discount_date_start':
						case 'product_discount_date_end':
						...
						case 'related_products':
							...
							if (strlen(trim($related_products)) == 0) $related_products = $default_values[$column_header];
							$contents .= $this->_exportmodel->AddExportField($related_products, $fieldname, $column_header);
							break;
						case 'product_discount':
							...
							if (strlen(trim($product_discount)) == 0) $product_discount = $default_values[$column_header];
							$contents .= $this->_exportmodel->AddExportField($product_discount, $fieldname, $column_header);
							break;
						case 'product_price':
							...
							if (strlen(trim($product_price)) == 0) $product_price = $default_values[$column_header];
							if ($template->add_currency_to_price) $product_price = $record->product_currency.' '.$product_price;
							$contents .= $this->_exportmodel->AddExportField($product_price, $fieldname, $column_header);
							break;
						case 'product_url':
							...
							if (strlen(trim($record->product_url)) == 0) {
								...
								if ($category_id > 0) {
									$product_url = JURI::root().JRoute::_('index.php?option=com_virtuemart&Itemid='.$vmitemid.'&page=shop.product_details&flypage='.$flypage.'&product_id='.$record->main_product_id.'&category_id='.$category_id, false);
								}
								else $product_url = "";
							}
							...
							$contents .= $this->_exportmodel->AddExportField($product_url, $fieldname, $column_header, true);
							break;
						case 'product_full_image':
							$product_full_image = str_replace($template->file_location, '', $record->product_full_image);
							$contents .= $this->_exportmodel->AddExportField($product_full_image, $fieldname, $column_header);
							break;
						case 'product_thumb_image':
							$product_thumb_image = str_replace($template->file_location, '', $record->product_thumb_image);
							$contents .= $this->_exportmodel->AddExportField($product_thumb_image, $fieldname, $column_header);
							break;
						case 'picture_url':
							...
							else $picture_url = $default_values[$column_header];
							$contents .= $this->_exportmodel->AddExportField($picture_url, $fieldname, $column_header);
							break;
						case 'price_with_tax':
							
							$price_with_tax = sprintf("%01.2f", $this->_exportmodel->ProductPrice(($record->product_price/100)*(100+($record->tax_rate*100))));
							/* Check if we have any content otherwise use the default value */
							if (strlen(trim($price_with_tax)) == 0) $price_with_tax = $default_values[$column_header];
							if ($template->add_currency_to_price) $price_with_tax = $record->product_currency.' '.$price_with_tax;
							$contents .= $this->_exportmodel->AddExportField($price_with_tax, $fieldname, $column_header);
							break;
						case 'price_with_discount':
							...
							/* Check if we have any content otherwise use the default value */
							if (strlen(trim($price_with_discount)) == 0) $price_with_discount = $default_values[$column_header];
							...
							$contents .= $this->_exportmodel->AddExportField($price_with_discount, $fieldname, $column_header);
							break;
						case 'product_packaging':
							$product_packaging = $record->product_packaging & 0xFFFF;
							$contents .= $this->_exportmodel->AddExportField($product_packaging, $fieldname, $column_header);
							break;
						case 'product_box':
							$product_box = $record->product_packaging>>16&0xFFFF;
							$contents .= $this->_exportmodel->AddExportField($product_box, $fieldname, $column_header);
							break;
						case 'product_name':
						...
						case 'product_desc':
							...
							if (strlen(trim($fieldvalue)) == 0) {
								...
								if (isset($record->product_s_desc) && $template->export_type == 'xml' && $template->export_site == 'froogle') {
									...
									/* Check if we have any content otherwise use the default value */
									$fieldvalue = $default_values[$column_header];
								}
						...
						case 'product_tax':
							/* Check if we have any content otherwise use the default value */
							if (strlen(trim($record->tax_rate)) == 0) $product_tax = $default_values[$column_header];
							else $product_tax = sprintf("%01.2f", ($record->tax_rate*100));
							$contents .= $this->_exportmodel->AddExportField($product_tax, $fieldname, $column_header);
							break;
						case 'mdate':
						case 'cdate':
