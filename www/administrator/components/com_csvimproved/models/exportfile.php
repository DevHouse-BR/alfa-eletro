...
class CsvimprovedModelExportfile extends JModel {
	...
	public function getExportFilename() {
		...
		if (!empty($template->export_filename)) {
			...
			if (!empty($template->file_location) && ($exportto == 'tofile')) {
				$localfile = $template->file_location.DS.$template->export_filename;
			}
			...
			if (!empty($template->file_location) && ($exportto == 'tofile')) {
				$localfile = $template->file_location.DS;
			}
	...
	public function getExportFields() {
		...
		foreach ($rows as $id => $row) {
			...
			if ($template->export_type == 'xml') {
				if (!empty($row->field_column_header)) $required_fields[str_replace(" ", "", $row->field_column_header)] = $row->field_name;
				else $required_fields[str_replace(" ", "", $row->field_name)] = $row->field_name;
			}
			else {
				if (!empty($row->field_column_header)) $required_fields[$row->field_column_header] = $row->field_name;
				else $required_fields[$row->field_name] = $row->field_name;
			}
			
			/* Collect the default values */
			if (!empty($row->field_column_header)) $default_values[$row->field_column_header] = $row->field_default_value;
			else $default_values[$row->field_name] = $row->field_default_value;
		}
		
		/* Return the required and default values */
		return array($required_fields, $default_values);
	}
	...
	public function ProductPrice($product_price) {
		for ($i=1;$i<4;$i++) {
			if (substr($product_price, -1) == 0) $product_price = substr($product_price, 0, -1);
			else $i = 4;
		}
	...
	public function writeOutput($contents) {
		...
		switch(JRequest::getVar('exportto')) {
			case 'todownload':
				...
				break;
			case 'tofile':
				if (fwrite(JRequest::getVar('handle'), utf8_encode($contents)) === FALSE) {
