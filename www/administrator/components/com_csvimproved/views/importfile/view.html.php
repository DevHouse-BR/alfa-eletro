...
class CsvimprovedViewImportFile extends JView {
						...
						if (JRequest::getVar('csvi_data') == false) {
							...
							if ($this->get('checklimits')) {
								...
								if ($this->get('Start', $this->template->template_type)) {
									...
									/* We have run into an error, do not continue processing */
									$processdata = false;
								}
	...
	function toolbar() {
		if ($this->template->show_preview && !JRequest::getVar('was_preview')) {
			JToolBarHelper::title(JText::_( 'Import preview' ), 'csvi_import_48');
		...
		else JToolBarHelper::title(JText::_( 'Import result' ), 'csvi_import_48');
	}
